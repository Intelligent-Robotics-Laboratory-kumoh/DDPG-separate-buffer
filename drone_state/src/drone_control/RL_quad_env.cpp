                         ...
                         float init_marker_x, float init_marker_y, int episode, int end_step)
{
    RL_env RL_;
    std::ofstream fout;
    std::string default_path = "/home/baek/traj_data";
    std::string traj_filename = "/traj";
    std::string traj_error_filename = "/error";

    if(!RL_.env_info.restart)
    {
        drone_traj[0][RL_.env_info.step] = pose_x; drone_traj[1][RL_.env_info.step] = pose_y; drone_traj[2][RL_.env_info.step] = pose_z;
        mlp_traj[0][RL_.env_info.step] = mlp_x; mlp_traj[1][RL_.env_info.step] = mlp_y; mlp_traj[2][RL_.env_info.step] = mlp_z;
    }
#ifndef TRACKING
    if(abs(RL_.real_x) <= 0.75 
    && abs(RL_.real_y) <= 0.75
    && abs(RL_.env_info.state.rel_pose[2]) <= 1
    && end_step > 1) write = true;
    else write = false;

    if(RL_.env_info.restart && (episode-past_episode > 0))
    {
        ROS_INFO("write : %d", write);
        fout.open(default_path+"/landingstate"+"/landingstate.txt",std::ios_base::app);
        if(fout.is_open())
        {
            fout<<"episode : "<<episode<<" landingstate : "<<write<<"\n";
        }
        fout.close();
    }
    past_episode = episode;
#else
    if(episode%100==0) write = true;
#endif
    //RL_.publish_logdata(pose_x,pose_y,pose_z,mlp_x,mlp_y,mlp_z,episode,RL_.env_info.restart,write,end_step);
    if(write)
    {
/*
        ...
        */
        std::stringstream current_episode;
        current_episode << episode;
        fout.open(default_path+"/traj"+traj_filename+"_"+current_episode.str()+".txt");
        if(fout.is_open())
        {
            for(int step=0;step<=end_step;step++)
            {
                fout<<"drone x: "<<drone_traj[0][step]<<" y: "<<drone_traj[1][step]<<" z: "<<drone_traj[2][step] <<" mlp x: "<<mlp_traj[0][step]<<" y: "<<mlp_traj[1][step]<<" z: "<<mlp_traj[2][step]+1.1;
                fout<<"\n";
            }
            fout.close();
        }

        fout.open(default_path+"/error"+traj_error_filename+"_"+current_episode.str()+".txt");
        if(fout.is_open())
        {
            for(int step=0;step<=end_step;step++)
            {
                fout<<"x: "<<mlp_traj[0][step]-drone_traj[0][step]<<" y: "<<mlp_traj[1][step]-drone_traj[1][step]<<" z: "<<mlp_traj[2][step]-drone_traj[2][step]+1.1;
                fout<<"\n";
            }
            fout.close();
        }
...
//                    RL_.SetModelvelReset(0,4.653,0);//0,4.653,0
    ...
    dataWrite(globalpose_.drone_x, globalpose_.drone_y, globalpose_.drone_z,
              ...
              init_target_x, init_target_y, RL_.env_info.episode,RL_.env_info.step);
}
