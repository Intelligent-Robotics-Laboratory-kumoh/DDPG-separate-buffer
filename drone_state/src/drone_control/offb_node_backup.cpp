...
float markerPixelGet(const geometry_msgs::PointStampedConstPtr &markerpixel)
{
    ...
    target.curr_x = ((iris_local_pose.z-mlp_gps_.alt)/400)*tan(veiw_angle) * (500-mark_pixel.x);// - (tan(veiw_angle)-tan(veiw_angle-angle.pitch)));
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        if(!land_trig)
        {
            ...
            {   
                pose.pose.position.x = iris_local_pose.x + target.curr_x * beta;
                pose.pose.position.y = iris_local_pose.y + target.curr_y * beta;
        ...
        {
            pose.pose.position.x = iris_local_pose.x + target.curr_x * beta/2;
            pose.pose.position.y = iris_local_pose.y + target.curr_y * beta/2;
            ...
            if(target_distance <= 0.5)
            {
                pose.pose.position.x = iris_local_pose.x + target.curr_x * beta/2;
                pose.pose.position.y = iris_local_pose.y + target.curr_y * beta/2;
                pose.pose.position.z = descent_flight(5,mlp_gps_.alt);
                local_pos_pub.publish(pose);
                ROS_INFO("ddddd");
                if(!markerrecogn)
                {
