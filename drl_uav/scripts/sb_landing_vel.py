...
class Critic:
    def __init__(self, state_size, action_size, name):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            self.fc1 = tf.compat.v1.layers.dense(self.concat, 1000, activation=None)#900
            self.bn1 = tf.nn.elu(tf.nn.batch_normalization(self.fc1,normalize_mean,normalize_val,shift,scale,val_epsilon))         
            self.fc2 = tf.compat.v1.layers.dense(self.bn1, 900, activation=None)#800
            self.bn2 = tf.nn.elu(tf.nn.batch_normalization(self.fc2,normalize_mean,normalize_val,shift,scale,val_epsilon))
            self.fc3 = tf.compat.v1.layers.dense(self.bn2, 800, activation=None)#700
...
#        self.critic_lr = 1e-2  # 5e-3 8e-8 8e-9
        self.tau = 5e-3 # 5e-3 #1E-5cv                 #21.10.18 tau : 3e-5 alr : 7e-9 clr : 7e-8
        self.actor_lr = 1e-5 # 1e-3 #normal1 ==> 1e-4 hidden layer 2 2E-6   0415 - 1e5
        self.critic_lr = 1e-4  # 5e-3 #normal1 ==> 1e-3 hidden layer 2 2E-5 0415 - 1e4
...
#        self.memory = deque(maxlen=self.MEMORY_CAPACITY)
        ...
        self.po_state_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, num_state))
        self.po_action_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, num_action))
        self.po_reward_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, 1))
        self.po_next_state_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, num_state))
        self.po_done_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, 1))

        self.ne_state_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, num_state))
        self.ne_action_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, num_action))
        self.ne_reward_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, 1))
        self.ne_next_state_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, num_state))  
        self.ne_done_buffer = numpy.zeros((self.MEMORY_CAPACITY_HALF, 1))

        self.ne_buffer_counter = 0
        self.po_buffer_counter = 0
    ...
    def observe(self, state, action, reward, next_state, done, step):
        if(step==1):
            self.positive_threshold = reward*0.75 # mlp speed 0.4 : 0.75
        
        obs_tuple = (state,action,reward,next_state,done)
        ne_index = self.ne_buffer_counter % self.MEMORY_CAPACITY_HALF
        po_index = self.po_buffer_counter % self.MEMORY_CAPACITY_HALF
        if(reward>self.positive_threshold):
            self.po_state_buffer[po_index] = obs_tuple[0]
            self.po_action_buffer[po_index] = obs_tuple[1]
            self.po_reward_buffer[po_index] = obs_tuple[2]
            self.po_next_state_buffer[po_index] = obs_tuple[3]
            self.po_done_buffer[po_index] = obs_tuple[4]
            self.po_buffer_counter += 1
        else :
            self.ne_state_buffer[ne_index] = obs_tuple[0]
            self.ne_action_buffer[ne_index] = obs_tuple[1]
            self.ne_reward_buffer[ne_index] = obs_tuple[2]
            self.ne_next_state_buffer[ne_index] = obs_tuple[3]
            self.ne_done_buffer[ne_index] = obs_tuple[4]
            self.ne_buffer_counter += 1
        print("positive buf length : "+str(self.po_buffer_counter)+" negative buf length : ",str(self.ne_buffer_counter))
    ...
    def replay(self, step):   # get knowledge from experience!
        global Envinfo, episode_R,  past_episode_R
        record_range = min(self.ne_buffer_counter+self.po_buffer_counter, self.MEMORY_CAPACITY_HALF)
        if record_range > 0:
            # Randomly sample indices
            if min(self.ne_buffer_counter,self.po_buffer_counter) < self.MEMORY_BATCH_SIZE*2:
                batch_indices = numpy.random.choice(record_range, self.MEMORY_BATCH_SIZE)
                state_batch = self.ne_state_buffer + self.po_state_buffer
                action_batch = self.ne_action_buffer + self.po_action_buffer
                reward_batch = self.ne_reward_buffer + self.po_reward_buffer
                next_state_batch = self.ne_next_state_buffer + self.po_next_state_buffer
                done_batch = self.ne_done_buffer + self.po_done_buffer

                state_batch = state_batch[batch_indices]
                action_batch = action_batch[batch_indices]
                reward_batch = reward_batch[batch_indices]
                next_state_batch = next_state_batch[batch_indices]
                done_batch = done_batch[batch_indices]

            else:
                eta = max(self.ne_buffer_counter,self.po_buffer_counter)/(self.ne_buffer_counter+self.po_buffer_counter)
                ne_batch_indices = numpy.random.choice(self.ne_buffer_counter, round(self.MEMORY_BATCH_SIZE*(1-eta)))
                po_batch_indices = numpy.random.choice(self.po_buffer_counter, round(self.MEMORY_BATCH_SIZE*eta))
                state_batch = numpy.concatenate((self.ne_state_buffer[ne_batch_indices],self.po_state_buffer[po_batch_indices]),axis=0)
                action_batch = numpy.concatenate((self.ne_action_buffer[ne_batch_indices],self.po_action_buffer[po_batch_indices]),axis=0)
                reward_batch = numpy.concatenate((self.ne_reward_buffer[ne_batch_indices],self.po_reward_buffer[po_batch_indices]),axis=0)
                next_state_batch = numpy.concatenate((self.ne_next_state_buffer[ne_batch_indices],self.po_next_state_buffer[po_batch_indices]),axis=0)
                done_batch = numpy.concatenate((self.ne_done_buffer[ne_batch_indices],self.po_done_buffer[po_batch_indices]),axis=0)

            states = normalize(state_batch)
            actions = normalize(action_batch)
            rewards = normalize(reward_batch)
            next_states = normalize(next_state_batch)
            ...
            target_qs = numpy.asarray([reward + self.GAMMA * (1 - done) * critic_target_predict_qs for reward, critic_target_predict_qs, done in zip(rewards, critic_target_predict_qs, done_batch)])
...
#        agent.deletestate(Envinfo.step)
    ...
    return getEnvinfoResponse(uav_vx = 2 * env_action[0], uav_vy = 2 * env_action[1], uav_vz = 1 * env_action[2])#, uav_yaw_sp = 1 * env_action[3])
...
#    env_action.thrust = 0
    writed = False
    update_start = False
    r = rospy.Rate(20)  # 30Hz

    while not rospy.is_shutdown():
        if(Envinfo.step != 0 and update_start==True):
            agent.replay(Envinfo.step) 
            update_start = False
        if Envinfo.done and Envinfo.reset_error == False and Envinfo.step > 1:
            if abs(Envinfo.rel_x<=0.5) and abs(Envinfo.rel_y)<=0.5 and abs(Envinfo.rel_z)<=1:
                landingstate = 1
            else:
                landingstate = 0
