...
static void mavlink_test_vision_target(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_VISION_TARGET >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_vision_target_t packet_in = {
        93372036854775807ULL,73.0,101.0,129.0,157.0,185.0,213.0,101
    };
    mavlink_vision_target_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.timestamp = packet_in.timestamp;
        packet1.XY_pgain = packet_in.XY_pgain;
        packet1.XY_igain = packet_in.XY_igain;
        packet1.XY_dgain = packet_in.XY_dgain;
        packet1.Z_pgain = packet_in.Z_pgain;
        packet1.Z_igain = packet_in.Z_igain;
        packet1.Z_dgain = packet_in.Z_dgain;
        packet1.vision_tracking_trig = packet_in.vision_tracking_trig;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_VISION_TARGET_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_VISION_TARGET_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_vision_target_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_vision_target_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_vision_target_pack(system_id, component_id, &msg , packet1.timestamp , packet1.XY_pgain , packet1.XY_igain , packet1.XY_dgain , packet1.Z_pgain , packet1.Z_igain , packet1.Z_dgain , packet1.vision_tracking_trig );
    mavlink_msg_vision_target_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_vision_target_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.timestamp , packet1.XY_pgain , packet1.XY_igain , packet1.XY_dgain , packet1.Z_pgain , packet1.Z_igain , packet1.Z_dgain , packet1.vision_tracking_trig );
    mavlink_msg_vision_target_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_vision_target_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_vision_target_send(MAVLINK_COMM_1 , packet1.timestamp , packet1.XY_pgain , packet1.XY_igain , packet1.XY_dgain , packet1.Z_pgain , packet1.Z_igain , packet1.Z_dgain , packet1.vision_tracking_trig );
    mavlink_msg_vision_target_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_vision_target(system_id, component_id, last_msg);
