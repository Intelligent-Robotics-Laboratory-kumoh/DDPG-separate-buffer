...
class PERMemory:
    """Prioritized Experience Replay 관리 클래스"""
    def __init__(self, capacity):
        self.tree = SumTree(capacity)
        self.capacity = capacity
        self.epsilon = 0.01  # 0이 되는 것을 방지
        self.alpha = 0.6     # 우선순위 반영 정도
        self.beta = 0.4      # Importance Sampling 가중치 시작값
        self.beta_increment_per_sampling = 0.001
        self.abs_err_upper = 5.0  # 초기 우선순위 최댓값
    ...
    def sample(self, n):
        ...
        # 확률 밀도 함수 기반 가중치 계산 준비
        min_prob = np.min(self.tree.tree[-self.tree.capacity:] + self.epsilon) / self.tree.total_p
        ...
        return batch_idx, batch_memory, np.array(is_weights)[:, np.newaxis]
...
class Actor:
    def __init__(self, state_size, action_size, name):
        with tf.compat.v1.variable_scope(name):
            ...
            # 네트워크 구조 유지
            fc1 = tf.compat.v1.layers.dense(self.state, 900, activation=tf.nn.elu)
            bn1 = tf.compat.v1.layers.batch_normalization(fc1)
            fc2 = tf.compat.v1.layers.dense(bn1, 800, activation=tf.nn.elu)
            bn2 = tf.compat.v1.layers.batch_normalization(fc2)
            fc3 = tf.compat.v1.layers.dense(bn2, 700, activation=tf.nn.elu)
            bn3 = tf.compat.v1.layers.batch_normalization(fc3)
            self.action = tf.compat.v1.layers.dense(bn3, action_size, activation=tf.nn.tanh)
...
class Critic:
    def __init__(self, state_size, action_size, name):
        with tf.compat.v1.variable_scope(name):
            ...
            fc1 = tf.compat.v1.layers.dense(concat, 900, activation=tf.nn.elu)
            bn1 = tf.compat.v1.layers.batch_normalization(fc1)
            fc2 = tf.compat.v1.layers.dense(bn1, 800, activation=tf.nn.elu)
            bn2 = tf.compat.v1.layers.batch_normalization(fc2)
            fc3 = tf.compat.v1.layers.dense(bn2, 700, activation=tf.nn.elu)
            bn3 = tf.compat.v1.layers.batch_normalization(fc3)
            self.predict_q = tf.compat.v1.layers.dense(bn3, 1, activation=None)
        self.trainable_var = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, name)
...
class Agent:
    def __init__(self, num_state, num_action):
        ...
        self.GAMMA = 0.99
#        self.MEMORY_BATCH_SIZE = 64    #260206 sepmem_fix_hb와 동일하게 변경
#        self.tau = 0.001 
...
#        self.EPSILON_DECAY = 0.9995
        ...
        self.tau = 1e-5 # 5e-3 # 0.4 m/s 5e-5 # 0.8 m/s deg 10 1e-5 
        self.actor_lr = 5e-7   # 1e-3 #normal1 ==> 1e-4 hidden layer 2 2E-6   0415 - 1e5  # 0.4 m/s 1e-5 # 0.8 m/s deg 10 5e-7
        self.critic_lr = 5e-6  # 5e-3 #normal1 ==> 1e-3 hidden layer 2 2E-5 0415 - 1e4  # 0.4 m/s 1e-4 # 0.8 m/s deg 10 5e-6
        ...
        # PER을 위해 MSE에 Importance Sampling 가중치(is_weights) 적용
    ...
    def replay(self):
        # 충분한 데이터가 쌓일 때까지 대기 (PER은 초기 트리가 채워질 시간이 필요함)
        if self.memory.tree.write < self.MEMORY_BATCH_SIZE and self.memory.tree.tree[0] == 0:
            return

        # PER 샘플링 (Index와 IS 가중치 추가 획득)
        ...
        next_s_batch = np.array([x[3] for x in mini_batch])/10000
        ...
        _, abs_errors = self.sess.run([self.train_critic, self.td_error], feed_dict={
            ...
            self.is_weights: is_weights # IS 가중치 주입
        })
...
def get_Envinfo_callback(data):
    ...
    return getEnvinfoResponse(uav_vx= 2 * env_action[0], uav_vy = 2 * env_action[1], uav_vz = 1 * env_action[2])
...
#    normalized_rel_pose_ydot = Envinfo.rel_pose_ydot / 20
    ...
    # # Z값을 -1(지면)에서 1(최고도) 사이로 명확히 매핑
    # z_range = MAXUAVPOSE_Z - MINUAVPOSE_Z
    # normalized_rel_z = 2.0 * (Envinfo.rel_z - MINUAVPOSE_Z) / z_range - 1.0
