...
{
    RL_env RL_;
    globalpose_.mlp_x = global_->pose[2].position.x;
    globalpose_.mlp_y = global_->pose[2].position.y;
    globalpose_.mlp_z = global_->pose[2].position.z;
    
    globalpose_.drone_x = global_->pose[3].position.x;
    globalpose_.drone_y = global_->pose[3].position.y;
    globalpose_.drone_z = global_->pose[3].position.z;
...
void RL_env::get_uavstate(offboard offb)
{
    ...
    env_info.state.yaw         = offb.angle.yaw;
}
...
void RL_env::exchange_stateNaction()
{
    ...
    clock_t et = clock();
    ROS_INFO("req & rep time : %f", double(et-st)/(CLOCKS_PER_SEC/1000));
}
...
float RL_env::calcul_reward(bool land_command, int step)
{
    offboard offb;

    float reward = 0;
    float reset_reward = 0;
    ROS_INFO("mark %d", offb.markerrecogn);
    if(mlp_distance >= 45) 
    {
     reset_reward = -1000;
    ...
    else if(env_info.state.rel_pose[2] < 0)
    {
     reset_reward = -1000; 
     ROS_WARN("relative z reset : %f, reward : %f",env_info.state.rel_pose[2],env_info.reward);
    }

    else if((abs(offb.angle.roll[0]*RAD2DEG)>=170)||(abs(offb.angle.pitch[0]*RAD2DEG)>=170))
    {
     reset_reward = -1000; 
     ROS_WARN("angle reset, roll : %f, pitch : %f, reward : %f",offb.angle.roll[0],offb.angle.pitch[0],env_info.reward);
    }

    else if(!offb.markerrecogn)
    {
     reset_reward = -1000;
     ROS_WARN("mark reset, markrecognization : %d, reward : %f",offb.markerrecogn,env_info.reward);
    }

    else reset_reward = 0;

    reward = (15 - (real_distance+env_info.state.rel_pose[2])*10) + reset_reward;
    return reward;
}
...
void offboard::gimbal_set()
{
    ...
    mount_configure.request.stabilize_roll = true;
    mount_configure.request.stabilize_pitch = true;
    mount_configure.request.stabilize_yaw  = true;
...
void offboard::iris_GPS_tracking()
{
    ...
    {
        //if(RL_.check_reset()) drone_state_ = RESET;
        drone_state_ = GPS_TRACKING;
    }
}
...
void RL_env::iris_RL()
{
    offboard offb;
    env_info.step++;
    env_info.reward = calcul_reward(true,env_info.step);
...
void offboard::Pub_targetpose()
{
    ...
    switch(drone_state_)
    {
        ...
        case RL :
        {              
            goal_vx = saturation(-12,12,RL_.env_info.action.vel[0]);
            goal_vy = saturation(-12,12,RL_.env_info.action.vel[1]);
            goal_vz = saturation( -3, 3,RL_.env_info.action.vel[2]);
            ROS_INFO("vx : %f, vy : %f, vz : %f", goal_vx,goal_vy,goal_vz);
        }
        break;
...
bool RL_env::check_reset()
{
    offboard offb;
    ROS_INFO("mlp_distance = %f", mlp_distance);
    if(mlp_distance >= 45) return true; 
    else if(!offb.markerrecogn)
    {
     ROS_WARN("mark reset : %d",offb.markerrecogn);
     return true;
    }
    else if(real_distance > 10) 
    {
     ROS_WARN("distance reset : %f",real_distance);
     return true;
    }
    else if(offb.iris_local_pose.z > 15)
    {
     ROS_WARN("altitude reset : %f",offb.iris_local_pose.z);
     return true;
    }
    else if(env_info.state.rel_pose[2] < 0)
    {
     ROS_WARN("relative z reset : %f",env_info.state.rel_pose[2]);
     return true;
    }
    else if((abs(offb.angle.roll[0]*RAD2DEG)>=170)||(abs(offb.angle.pitch[0]*RAD2DEG)>=170))
    {
     ROS_WARN("angle reset, roll : %f, pitch : %f",offb.angle.roll[0],offb.angle.pitch[0]);
     return true;
    }
    else if(env_info.step > MAXSTEP) 
    {
     ROS_WARN("step reset : %f",env_info.step);
     return true; //MAXSTEP 
    }
    else return false;
}
...
int offboard::run()
{
    ...
    switch(drone_state_)
    {
        ...
        case RESET :
        {
            RL_.success_region_step = 0;
            RL_.SetModelReset(false,"iris", 0.0, -5.0, 9.3, 0.0, 0.0, 0.0);
            if(RL_.reset_client.call(RL_.reset))
            {
                RL_.SetModelReset(false,"Mobile_Leveling_Platform_V2", 0.0, 0.0, 0.27);
                if(RL_.reset_client.call(RL_.reset)) drone_state_ = STABLE;
                else 
                {
                    drone_state_ = RESET;
            ...
            RL_.env_info.step = 0;
            RL_.env_info.episode++;
            if(RL_.env_info.episode > MAXEPISODE) drone_state_ = OFF;
        }
        break;

        case STABLE :

            pub_pose(iris_local_pose.x,iris_local_pose.y,9.4);
            if(current_state.mode != "OFFBOARD")
            {
                offb_set_mode.request.custom_mode = "OFFBOARD";
                set_mode_client.call(offb_set_mode);
            ...
            if((abs(abs(iris_local_pose.z)-9.4) < 0.05))
            {
                //RL_.SetModelvelReset(4.653,0,0);
                drone_state_ = INIT;
            }
            else 
            {
                RL_.SetModelReset(false,"iris", 0,-5, 9.4, 0.0, 0.0, 0.0); //RL_.save_iris_x,RL_.save_iris_y
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        offb.run();
        clock_t et = clock();
        ROS_INFO("time per run : %f", double(et-st)/(CLOCKS_PER_SEC/1000));
        ros::spinOnce();
        rate.sleep();
    }
