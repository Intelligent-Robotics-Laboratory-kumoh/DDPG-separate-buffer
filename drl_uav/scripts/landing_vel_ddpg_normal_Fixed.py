...
class Actor:
    def __init__(self, state_size, action_size, name):
        with tf.compat.v1.variable_scope(name):
            ...
            # 네트워크 구조 유지
            fc1 = tf.compat.v1.layers.dense(self.state, 400, activation=tf.nn.relu)
            bn1 = tf.compat.v1.layers.batch_normalization(fc1)
            fc2 = tf.compat.v1.layers.dense(bn1, 300, activation=tf.nn.relu)
...
class Critic:
    def __init__(self, state_size, action_size, name):
        with tf.compat.v1.variable_scope(name):
            ...
            fc1 = tf.compat.v1.layers.dense(concat, 400, activation=tf.nn.relu)
            bn1 = tf.compat.v1.layers.batch_normalization(fc1)
            fc2 = tf.compat.v1.layers.dense(bn1, 300, activation=tf.nn.relu)
