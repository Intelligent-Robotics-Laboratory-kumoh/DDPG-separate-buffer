...
from drl_uav.srv import getLandingState, getLandingStateResponse           # push action
...
#            self.bn0 = tf.nn.selu(tf.nn.batch_normalization(self.state,normalize_mean,normalize_val,shift,scale,val_epsilon))            
            self.fc1 = tf.compat.v1.layers.dense(self.state, 600, activation='elu')
            self.bn1 = tf.nn.elu(tf.nn.batch_normalization(self.fc1,normalize_mean,normalize_val,shift,scale,val_epsilon))
            self.fc2 = tf.compat.v1.layers.dense(self.bn1, 500, activation='elu')
            self.bn2 = tf.nn.elu(tf.nn.batch_normalization(self.fc2,normalize_mean,normalize_val,shift,scale,val_epsilon))
            self.fc3 = tf.compat.v1.layers.dense(self.bn2, 400, activation='elu')
            self.bn3 = tf.nn.elu(tf.nn.batch_normalization(self.fc3,normalize_mean,normalize_val,shift,scale,val_epsilon))        
#            self.fc4 = tf.compat.v1.layers.dense(self.bn3, 700, activation=None)
...
#            self.bn0 = tf.nn.selu(tf.nn.batch_normalization(self.concat,normalize_mean,normalize_val,shift,scale,val_epsilon))
            self.fc1 = tf.compat.v1.layers.dense(self.concat, 600, activation='elu')
            self.bn1 = tf.nn.elu(tf.nn.batch_normalization(self.fc1,normalize_mean,normalize_val,shift,scale,val_epsilon))         
            self.fc2 = tf.compat.v1.layers.dense(self.bn1, 500, activation='elu')
            self.bn2 = tf.nn.elu(tf.nn.batch_normalization(self.fc2,normalize_mean,normalize_val,shift,scale,val_epsilon))
            self.fc3 = tf.compat.v1.layers.dense(self.bn2, 400, activation='elu')
...
#        self.critic_lr = 1e-2  # 5e-3 8e-8 8e-9
        ...
        self.actor_lr = 8e-8 # 1e-3 #normal1 ==> 1e-4 hidden layer 2 2E-6
        self.critic_lr = 8e-7  # 5e-3 #normal1 ==> 1e-3 hidden layer 2 2E-5
...
#            if Envinfo.episode % 500 == 0 :
#                self.saver.save(self.sess, 'saved_networks/' +'tracking/'+ 'network' + '-ddpg_trackinging', global_step = Envinfo.episode)
...
def get_Envinfo_callback(data):
    global Envinfo, env_action, agent, state, next_state, episode_R, update_start
    ...
    return getEnvinfoResponse(uav_vx = 5 * env_action[0], uav_vy = 5 * env_action[1])#, uav_vz = 1 * env_action[2], uav_yaw_sp = env_action[3])
...
#    normalized_rel_z = 2*(Envinfo.rel_z-3.5)  / (MAXUAVPOSE_Z-MINUAVPOSE_Z)    
    normalized_rel_pose_xdot = Envinfo.rel_pose_xdot / (MAXUAVPOSE_Z*math.tan(camera_fov/2))
    normalized_rel_pose_ydot = Envinfo.rel_pose_ydot / (MAXUAVPOSE_Z*math.tan(camera_fov/2))
...
#    state_ = numpy.array((normalized_rel_x,normalized_rel_y,normalized_rel_z,normalized_rel_pose_xdot,normalized_rel_pose_ydot,normalized_yaw))
    state_ = numpy.array((normalized_rel_x,normalized_rel_y,normalized_rel_pose_xdot,normalized_rel_pose_ydot,normalized_yaw))
#    print(state_)
