...
class Model:
    def __init__(self, state_size, action_size):
        self.state = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, state_size])
        self.fc1 = tf.compat.v1.layers.dense(self.state, 32, activation=tf.nn.relu, kernel_initializer=tf.initializers.he_uniform(1))
        self.fc2 = tf.compat.v1.layers.dense(self.fc1, 32, activation=tf.nn.relu, kernel_initializer=tf.initializers.he_uniform(1))
        self.fc3 = tf.compat.v1.layers.dense(self.fc2, 32, activation=tf.nn.relu, kernel_initializer=tf.initializers.he_uniform(1))
        self.q_value = tf.compat.v1.layers.dense(self.fc3, action_size)
        self.action = tf.compat.v1.arg_max(self.q_value, 1)
       
        self.target_q = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, action_size])
        self.loss = tf.losses.mean_squared_error(self.target_q, self.q_value)
        self.model_update = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(self.loss)
        self.trainable_var = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES)
...
"""class Brain:
    ...
    def set_weights(self, weight):
        return self.model.set_weights(weight)

    def _createModel(self): # model: state -> v(state value)
        model = Sequential()

        model.add(Dense(128, activation='relu', input_dim=self.num_state))
        # model.add(Dense(128, activation='relu'))
        #model.add(Dense(128, activation='relu'))
        model.add(Dense(self.num_action, activation='relu'))
    ...
    def train(self, x, y, batch_size=128, epochs=1, verbose=0):
...
class Agent:
    ...
    def __init__(self, num_state, num_action):
        ...
        """self.main_brain = DQN(sess, self.num_state, self.num_action, name="main")
        self.target_brain = DQN(sess, self.num_state, self.num_action, name="target")
        sess.run(tf.compat.v1.global_bariables_initializer())
        copy_ops = get_copy_var_ops(dest_scope_name="target", src_scope_name="main")
        sess.run(copy_ops)"""
        #self.brain = Brain(num_state, num_action, RL_GAMMA=self.GAMMA)
        #self.target_brain = Brain(num_state, num_action, RL_GAMMA=self.GAMMA)
        #self.target_model = self.model
        self.target_update_rate = 20
        #self.update_target_model()
        self.memory = deque(maxlen=self.MEMORY_CAPACITY)
        self.main_brain = Model(self.num_state, self.num_action)
        self.target_brain = Model(self.num_state, self.num_action)
        self.sess=tf.compat.v1.Session()
        self.sess.run(tf.compat.v1.global_variables_initializer())

        self.update_target = []
        for i in range(len(self.main_brain.trainable_var)):
            self.update_target.append(self.target_brain.trainable_var[i].assign(self.main_brain.trainable_var[i]))
        self.sess.run(self.update_target)



    """def get_copy_var_ops(*, dest_scope_name: str, src_scope_name: str) -> list:
        op_holder = []
    ...
    def soft_target_update(self):
        ...
        copy_ops = TU.get_copy_var_ops(dest_scope_name="target", src_scope_name="main")
        sess.run(copy_ops)
    ...
    def act(self, state):   # action:[0,1,2,...,num_action-1]
        ...
        if(numpy.random.rand() < self.epsilon):
            ...
            return numpy.random.randint(0, self.num_action)
        else:
            return self.sess.run(self.main_brain.action, feed_dict={self.main_brain.state: [state]})[0]

        return action

        """if random.random() < self.epsilon:
            action_random=random.randint(1, 7)
            action_random=action_random/6.0
            return action_random
        else:
            action_train = numpy.argmax(self.brain.predictOne(state_test=state))  # get the index of the largest number, that is the action we should take. -libn
            action_train=action_train/6.0
            return action_train"""
    ...
    def observe(self, state, action, reward, next_state, done):
        ...
        self.steps += 1
        """if (self.steps % self.target_update_rate) == 0:
            with tf.compat.v1.Session() as sess:
                sess.run(tf.compat.v1.global_variables_initializer())
                copy_ops = TU.get_copy_var_ops(dest_scope_name="target", src_scope_name="main")
                sess.run(copy_ops)"""
    ...
    def replay_train(self):
        if len(self.memory) > 5000:
            ...
            states = numpy.zeros((self.MEMORY_BATCH_SIZE, self.num_state))
            next_states = numpy.zeros((self.MEMORY_BATCH_SIZE, self.num_state))
            ...
            target = self.sess.run(self.main_brain.q_value, feed_dict={self.main_brain.state: states})
            target_val = self.sess.run(self.target_brain.q_value, feed_dict={self.target_brain.state: next_states})
            #print(target)
            for j in range(self.MEMORY_BATCH_SIZE):
                #print('1111111111111111111111111111111111111111111111111111111111111111')
                #print(target_val)
                target[j][actions[j]] = rewards[j] + ((1 - dones[j]) * (self.GAMMA * numpy.amax(target_val[j])))
                #print(target)
            self.sess.run([self.main_brain.loss, self.main_brain.model_update], feed_dict={self.main_brain.state: states, self.main_brain.target_q: target})
    ...
    """def replay(self):   # get knowledge from experience!
        ...
        v_ = self.target_brain.predict(batch_states_)
        ...
        if len_batch > 2000:
            self.brain.train(x, y, batch_size=128)
"""
# get UAV status:
...
def interact():
    ...
    #reward = 5*numpy.exp(-(1/16)*(UAV_Pos.pose.position.z - 20)**2)
...
def main_loop():
    ...
    while not rospy.is_shutdown():
        ...
        if(att_running.running):
            ...
            if((n % agent.target_update_rate) == 0):
