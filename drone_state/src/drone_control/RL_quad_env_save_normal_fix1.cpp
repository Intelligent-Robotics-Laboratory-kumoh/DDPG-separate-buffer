...
std::string getLatestSessionDir() {
    std::string base_path = "/home/baek/ddpg_result/ddpg_normal";
    DIR *dir;
    struct dirent *ent;
                         ...
                         float arg_step, float arg_restart, int episode, int end_step)
{
    ...
    if(is_restart && (episode != last_saved_episode)) {
        ...
        if(fout.is_open()){
            ...
            bool is_success = (sqrt(pow(mlp_x - pose_x, 2) + pow(mlp_y - pose_y, 2)) <= 0.75);
...
float RL_env::calcul_reward(offboard offb, int step, bool negative = true)
{
    ...
    // 1. 상태 변수 정의
    float dist_xy = sqrt(pow(real_x, 2) + pow(real_y, 2));
    float dist_z = abs(env_info.state.rel_pose[2]);
    float dist_3d = sqrt(pow(dist_xy, 2) + pow(dist_z, 2));
    
    // 상대 속도 (드론과 플랫폼 간의 속도 차이)
    float rel_vel_x = offb.iris_vel.x - offb.globalvel_.mlp_x;
    float rel_vel_y = offb.iris_vel.y - offb.globalvel_.mlp_y;
    float rel_vel_norm = sqrt(pow(rel_vel_x, 2) + pow(rel_vel_y, 2));

    // --- A. 강제 종료 및 실패 조건 ---

    // 1) 마커를 인식하지 못한 경우 (실패로 간주)
    ...
    // --- B. 최종 성공 조건 (기존 좌표 조건 사용) ---

    if (abs(real_x) <= 0.75f && abs(real_y) <= 0.75f && env_info.state.rel_pose[2] <= 1.0f) {
        reward = 10000.0f; // 기존 코드와 동일한 보상값
        ROS_WARN("Success! Goal Reached. Reward: %f", reward);
        
        env_info.restart = true;
        offb.write = true; // 데이터 저장 플래그 활성화
        return reward;
    }

    // --- C. 지속적 셰이핑 보상 (학습 가속화) ---
    
    // 거리 기반 지수 보상 (목표에 가까워질수록 증가)
    float dist_reward = 2.0f * exp(-0.5f * dist_3d); 
    
    // 속도 동기화 보상 (플랫폼과 속도가 일치할수록 증가)
    float vel_reward = 1.0f * exp(-0.5f * rel_vel_norm);

    // 하강 정렬 페널티 (낮은 고도에서 수평 오차가 클수록 강한 페널티)
    float alignment_penalty = 0;
    if (dist_z < 2.0f) {
        alignment_penalty = -5.0f * dist_xy; // 정밀 착륙 유도
    }

    reward = dist_reward + vel_reward + alignment_penalty;

    // 셰이핑 리워드 모니터링
    ROS_INFO("Step: %d, Shaping Reward: %f, Dist_XY: %f, Dist_Z: %f", step, reward, dist_xy, dist_z);
